...
public:
  ...
  void receive(const Token &TokP) override {
    Token Tok = TokP;
...
Parser::Parser(std::unique_ptr<Lexer> Lex, SourceFile &SF,
  ...
  if (SPActions == nullptr) {
    SPActions = std::make_shared<SyntaxTreeCreator>(
...
Parser::parseMatchingTokenSyntax(tok K, Diag<> ErrorDiag, SourceLoc OtherLoc,
  ...
  // We did not consume the token. Error path.
  ...
  if (!SilenceDiag) {
    ...
    switch (K) {
    case tok::r_paren:
      OtherNote = diag::opening_paren;
      break;
    case tok::r_square:
      OtherNote = diag::opening_bracket;
      break;
    case tok::r_brace:
      OtherNote = diag::opening_brace;
      break;
    default:
