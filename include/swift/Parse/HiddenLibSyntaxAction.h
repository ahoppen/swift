...
public:
  ...
  OpaqueSyntaxNode recordToken(tok tokenKind, StringRef leadingTrivia,
    ...
    if (ExplicitAction == LibSyntaxAction) {
      ...
      libSyntaxActionNode = LibSyntaxAction->recordToken(
  ...
  recordRawSyntax(syntax::SyntaxKind kind,
    ...
    if (ExplicitAction) {
      if (ExplicitAction == LibSyntaxAction) {
        explicitActionNode = ExplicitAction->recordRawSyntax(kind, elements);
      } else {
        ...
        for (auto element : elements) {
          ...
          if (element) {
            explicitActionElement =
                static_cast<const HiddenNode *>(element)->ExplicitActionNode;
          }
    ...
    if (ExplicitAction == LibSyntaxAction) {
      ...
      if (ExplicitAction == nullptr) {
        libSyntaxActionNode = LibSyntaxAction->recordRawSyntax(kind, elements);
      } else {
        ...
        libSyntaxActionNode =
            LibSyntaxAction->recordRawSyntax(kind, libSyntaxActionElements);
      }
    }
  ...
  OpaqueSyntaxNode makeDeferredLayout(
    ...
    if (ExplicitAction) {
      if (ExplicitAction == LibSyntaxAction) {
        ...
        SmallVector<RecordedOrDeferredNode, 4> explicitActionChildren;
        explicitActionChildren.reserve(children.size());
        for (auto child : children) {
          RecordedOrDeferredNode explicitActionChild(nullptr, child.getKind());
          if (child.getOpaque()) {
            explicitActionChild = RecordedOrDeferredNode(
                getExplicitNodeFor(child.getOpaque()), child.getKind());
          }
    ...
    if (ExplicitAction == LibSyntaxAction) {
      ...
      if (ExplicitAction == nullptr) {
        ...
        SmallVector<RecordedOrDeferredNode, 4> libSyntaxActionChildren;
        libSyntaxActionChildren.reserve(children.size());
        for (auto child : children) {
          RecordedOrDeferredNode libSyntaxActionChild(nullptr, child.getKind());
          if (child.getOpaque()) {
            libSyntaxActionChild = RecordedOrDeferredNode(
                getLibSyntaxNodeFor(child.getOpaque()), child.getKind());
          }
  ...
  CharSourceRange getDeferredChildRange(OpaqueSyntaxNode node,
    ...
    CharSourceRange libSyntaxNumChildren =
        LibSyntaxAction->getDeferredChildRange(getLibSyntaxNodeFor(node),
                                               childIndex, startLoc);
    if (ExplicitAction) {
      assert(ExplicitAction->getDeferredChildRange(getExplicitNodeFor(node),
                                                   childIndex, startLoc) ==
             libSyntaxNumChildren);
    }
    return libSyntaxNumChildren;
  }

  size_t getDeferredNumChildren(OpaqueSyntaxNode node) override {
    size_t libSyntaxNumChildren =
        LibSyntaxAction->getDeferredNumChildren(getLibSyntaxNodeFor(node));
    if (ExplicitAction) {
      assert(ExplicitAction->getDeferredNumChildren(getExplicitNodeFor(node)) ==
             libSyntaxNumChildren);
    }
    return libSyntaxNumChildren;
  }
